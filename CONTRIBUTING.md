...
**From a fresh clone, run `INSTALL.bat` first.** A checkout is source only: the
embedded runtime (6.4 GB) and FFmpeg (385 MB, two 193 MB binaries) cannot live in
git — GitHub hard-rejects any file over 100 MB. `INSTALL.bat` fills both gaps: it
installs Python 3.11 if the machine has none, then the dependencies you tick, and
downloads FFmpeg when `backend/FFmpeg` is absent. Afterwards:

```bat
RUN.bat        :: desktop client
SERVER.bat     :: web cabinet on :8000
```

Those two launchers work unchanged in a clone, in a `min` install and in an
unpacked `full` build, because they look for the interpreter at run time: the
embedded runtime first, then the one `INSTALL.bat` recorded in
`config/interpreter.txt`, then `PATH`.

Running a module directly is fine too, with whichever interpreter you installed
into — in a `full` build that is the embedded one:
...
Run all of them (53 runners, 1786 checks, a few minutes):
