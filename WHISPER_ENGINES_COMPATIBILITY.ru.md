...
### Live-транскрибация (потоковая)

В live-режиме модель загружается ОДИН раз на всю встречу и распознаёт короткие
реплики по мере речи (`backend/processing/live_engines.py`), а не грузится на
каждый прогон и не читает файлы. Потоковый адаптер есть у шести движков из семи:

| Движок | Live | Примечание |
|---|---|---|
| `faster-whisper` | да | По умолчанию. `beam_size=1` — beam search на трёхсекундной реплике стоит задержки, которую видно, и даёт точность, которую нет |
| `whisperx` | да | Работает на своих весах faster-whisper. **Без диаризации в live** — это батчевый этап, в двухсекундной реплике искать структуру говорящих не в чем |
| `whisper` | да | Эталонное качество, заметно медленнее на реплику |
| `vosk` | да | Потоковый по своей природе, самый лёгкий вариант для процессора |
| `sherpa-onnx` | да | Быстрый на процессоре — разумный выбор, когда видеокарта занята локальной LLM |
| `whisper-cpp` | да | Экономный к процессору ggml |
| `funasr` | нет | Декодирует окна фиксированной формы, для задержки меньше секунды не годится. При выборе для live выдаётся понятная ошибка, а не молчаливая подмена движка |

Собственный VAD движка в live выключен: поток уже нарезан на реплики выше по
цепочке, а второй VAD поверх двухсекундного фрагмента срезает тихие окончания.
Словарь терминов (`transcriptionHint`) в live whisper-семейству **передаётся** —
именно он сохраняет имена и аббревиатуры.
