...
_TRUNCATED_REASONS = {"length", "max_tokens", "max_token", "model_length",
                      "output_limit"}


def _warn_if_truncated(finish_reason, provider, limit):
    """Say out loud that the model ran out of output budget mid-answer.

    Every provider reports this ('length' / 'max_tokens' / 'MAX_TOKENS'), and
    nothing here used to look. A cut-off answer travels on as if it were
    complete, so an analysis feature whose JSON array stops mid-question is
    reported to the user as "AI returned invalid JSON/schema" — which blames
    the model's formatting for what is a token-budget problem. stderr reaches
    the app's error text, so the real cause is visible where the failure shows.
    """
    if str(finish_reason or "").strip().lower() in _TRUNCATED_REASONS:
        print(f"WARNING: {provider} stopped at the output ceiling of {limit} "
              f"tokens — the answer is CUT OFF mid-sentence, not complete. "
              f"Raise --max-tokens (or shorten the input) if sections come back "
...
class AIClient:
    ...
    def _call_openai_compatible(self, base_url, text):
        ...
        try:
            ...
            _warn_if_truncated(choice.get("finish_reason"), self.provider,
                               self.max_tokens)
    ...
    def _call_local_once(self, base, text):
        ...
        try:
            ...
            response.raise_for_status()
            choice = response.json()["choices"][0]
            _warn_if_truncated(choice.get("finish_reason"), "Local model",
                               self.max_tokens)
    ...
    def _call_anthropic(self, text):
        try:
            ...
            response.raise_for_status()
            data = response.json()
            _warn_if_truncated(data.get("stop_reason"), "Anthropic", self.max_tokens)
    ...
    def _call_google(self, text):
        ...
        try:
            ...
            _warn_if_truncated(candidate.get("finishReason"), "Google",
                               self.max_tokens)
