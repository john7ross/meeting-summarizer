...
Stats and the on-demand local-AI manager.
...
- `atomic_io` protects `config/history.json` and `config/settings.json` with an
...
| --- | --- | --- | --- |
| Desktop | `config/settings.json`, `config/history.json` | `transcripts/<sanitized file name>/` | isolated: `rag_knowledge_base/`; shared: `rag_shared/<key sha256>/` |
...
7. One AI call is made per enabled analysis feature. The default source is the transcript;
   the optional summary source records its version as provenance.
8. The merged analysis is versioned. Best-effort Obsidian/Google Sheets export and manual
   RAG indexing happen without changing pipeline success if an integration fails.

Editing a transcript and choosing **Regenerate** skips ASR and creates new summary and
analysis versions; prior versions remain selectable.
