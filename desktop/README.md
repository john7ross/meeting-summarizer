...
- **Add by URL** — paste a YouTube or file-server link; the video is downloaded (yt-dlp, with
  ...
  uploaded file.
- **Record from the microphone** — a built-in recorder (🎙 in the header) captures a meeting
  live straight to 16 kHz mono WAV (device picker, level meter, pause). On stop the recording
  enters the normal flow: trim into segments, then the queue.
...
- **Advanced analysis** — 11 features (action items, sentiment, category, risks, quotes,
  ...
  are explicit speed/context-window trade-offs.
...
- **Ops** — version history + regenerate, CUDA auto-detect with CPU fallback, a Diagnostics
  window (system metrics / processing profile / engine A-B / logs), and a Stats modal.
