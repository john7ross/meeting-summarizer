...
## Live mode, and the half of every online meeting we were not recording (2026-08-21) — v1.3.0

Prompted by a comparison with a work tool (Mic Recorder 1.9.0, an Electron client whose
sources ship unpacked; the full teardown is in `MIC_RECORDER_VS_MEETING_SUMMARIZER.md`).
It does two things we did not — live transcription and a live summary — and one thing
nobody had asked for but which turns out to matter more: it records **system audio**. Our
recorder captured the microphone only, so every recording of an online meeting held one
side of the conversation. That is the defect the whole release hangs off; live transcription
of half a call would have been a worse feature, not a better one.

**Three modules in the backend, because live is not batch with a flag on it.**
`processor.py` is file-in/file-out and loads a model per run; live is stream-in/event-out and
must hold ONE model for the whole meeting. Sharing the module would mean two lifecycles in
one file, so `live_stt.py` is separate — PCM on stdin, JSON lines on stdout, the same
"front end assembles argv and reads lines" contract everything else uses. What IS shared is
what matters: model resolution goes through `engines_registry`, so an engine whose model is
downloaded works live with no second copy and no second setting. Six of seven engines have a
streaming adapter (`processing/live_engines.py`); FunASR decodes fixed-shape windows and
raises a clear error rather than being silently swapped for another engine.

**Decoding runs behind a bounded queue on its own thread.** If it did not, a slow model
would block the read from stdin, the pipe would fill, and the recorder — on the UI thread —
would stall. Recording must never pay for transcription being slow.

**The segmenter is energy VAD with two things a naive one lacks** (`processing/live_vad.py`,
stdlib only, so it is testable without a microphone or a model): an adaptive noise floor
(a fan or a noisy headset otherwise makes it deaf or permanently triggered) and hysteresis —
speech starts at a higher threshold than it stops at, because with one threshold the tail of
every sentence, which is quieter than its middle, gets clipped and the engine receives
"давайте перенесём" without "на пятницу". Plus a pre-roll ring buffer, since by the time the
level crosses the trigger the first consonant is already gone.

**Two capture sources give speaker attribution for free.** Mic left, system right, in one
stereo WAV; the segmenter compares per-channel energy over each utterance and labels it
MIC/SYSTEM, saying "mix" honestly when the two are within a few dB. Both downstream
consumers already force `-ac 1`, so the batch transcript sees a correct downmix and nothing
on their side changed. The microphone stays the clock: `_mix_with_system` pulls exactly as
many loopback samples as the mic just delivered, padding with silence when the loopback
device is behind and dropping the oldest when it runs ahead. Two independent devices never
tick at the same rate and that drift has to be absorbed somewhere; absorbing it against the
mic keeps the two channels aligned with each other, which is the only alignment anyone hears.

**Loopback capture is optional and says why when it is not there.** `soundcard` is a pure-
Python wheel over the cffi/numpy already installed, so it costs nothing — but a machine
without it, or without a loopback-capable output, records mic-only and reports which of the
two it is. Verified for real: the self-test plays a tone through the default output and
asserts the loopback stream captured it, rather than asserting the API returned without error.

**The live summary reuses `ai_client.py` and therefore all four ways to attach an AI.**
`live_summary.py` owns only the state (topics, decisions, action items, open questions,
entities) and the prompts that maintain it; the call itself is `AIClient`. Nothing about
"which AI" is configured twice.

**The update strategy is chosen per provider, not once for everyone.** An incremental update
builds on its own previous answer, so a mistake at minute 3 is quoted back to the model for
the rest of the meeting. Mic Recorder's own design notes work this through, conclude "full
regen from the transcript, anti-drift", and then ship the increment anyway with a
consolidation pass as a patch — consolidation tidies the state, it cannot know the state is
wrong. Here: a local model rebuilds from the transcript every time (tokens cost only time),
a metered cloud alternates increments with periodic rebuilds and consolidation, and
`liveSummaryMaxUpdates` caps the spend. The user can override the choice.

**Never blank a good summary.** A failed request, a killed process, an answer that will not
parse, or an answer that empties a summary which had content — all of them leave the last
good text exactly where it is and put the complaint on a separate status line. The status is
a sibling widget, not a replacement for the text; writing "updating..." into the panel is how
a live summary appears to vanish every time it refreshes.

**A live meeting is processed from its own transcript, and produces the same artifacts as
everything else.** Live already recognised the speech; transcribing the file again afterwards
is minutes of GPU for a result the meeting already had. *Process from live text* writes the
live transcript into the job folder under the same `<stem>_raw.txt` name a transcription run
would have written and calls the existing `start_from_transcript` path — the one the Regenerate
button uses — so summary and analysis run exactly as they always do. The constraint that shaped
this: **the artifact set must not depend on how the input arrived.** So the live SUMMARY is
deliberately NOT injected as summary v1 — that would leave live meetings with two summary
versions and file meetings with one, and "how was this recorded" would leak into the version
pickers, the exports and the Obsidian notes. The live summary stays what it is: a side artifact
next to the recording. `_selftest_pipeline.py` compares a live-sourced meeting against a
batch-sourced one artifact for artifact, by count and by name.

Trimming is skipped on that route, for a reason worth writing down: the live transcript covers
the WHOLE recording, so cutting the audio into per-meeting segments would give every segment a
transcript containing all the others.

**`live` became an intake channel, not a hidden special case.** A meeting arrives as a file, a
URL or live, and `HistoryEntry.source` records which — persisted, round-tripped, and defaulting
to `file` for every entry written before this release (an unknown-channel value would be a lie:
all of them came from files). It is badged in the queue row, named in the results panel, carried
into the processing journal's own record, logged when the job starts, and the Diagnostics
profile renders the stages that really ran — transcription is absent there because it really
did not run, not because the profile is broken. The i18n suite caught the one thing that is easy
to get wrong here: a label rendered from DATA has to be re-rendered on a language switch, not
just re-labelled.

**Version display and version drift, both found while doing this.** The desktop client showed
its version nowhere — the web footer had it, exported reports had it, and a user reporting a
bug from the desktop could not name their build. It is now in Diagnostics → System. And
nothing compared the version claimed in the DOCS (two README badges, two desktop READMEs,
PROJECT_OVERVIEW) with `package.json`; a bump could ship an archive named v1.3.0 whose README
said 1.2.1. `_selftest_build_composition.py` now checks all five, plus that both release-note
files actually cover the version being shipped — the latter failed on the first run, exactly
as intended, until the notes were written.

**Verified, not assumed.** Real Russian speech through the whole chain: sherpa-onnx ~0.1 s
per utterance on the CPU, faster-whisper `medium` ~0.6 s on the GPU, both honouring the
vocabulary hint (which Mic Recorder passes as `prompt: ''` and thus wastes). Two new runners,
both against real subprocesses rather than mocks — `_selftest_live.py` (82) and
`_selftest_live_ui.py` (100). Suite: **53 runners, 1786 checks, zero failures.** Version
raised to **1.3.0** — this is a feature, not a debug pass.

**Not done here, deliberately:** the web cabinet. The backend modules are front-end agnostic
and `ConnectionManager` already exists to carry live events over the WebSocket, but the
browser capture story is its own problem and desktop-first keeps this release shippable.

## The JSON recovery was counting brackets inside strings (2026-08-19)

**From the owner's own run of three meetings on 2026-08-19:** two finished, and
`2026-08-18 15-00-40.mkv` died at 94% with "ошибок 1 из 11" on the `questions`
feature — taking its Obsidian note and its Sheets row with it, although the other ten
features had succeeded and their results were already in the artifact. The logged
response begins `[ { "question": "Необходимо провести внутреннее аутентифицированное
сканирование…` — a well-formed array. The parser returned `None` on it anyway.

Two independent defects, both reproduced deterministically before touching anything:

* `parse_json_response` compared `cleaned.count("{")` with `cleaned.count("}")` (and
  the same for brackets) **before** trying `json.loads` at all, so the count included
  every delimiter inside a string VALUE. One unmatched `[` or `{` in a question's own
  text — "п. [3", "формат { отчёта" — sent an intact answer straight into the lossy
  repair path. `_selftest_json_recovery.py` fails 5 of 11 checks against the old
  implementation and passes all 11 against the new one.
* `_fix_incomplete_json` could not close an array of objects: it appended every `]`
  before every `}`. Its trim-back also cut at `rfind(",")`, and in Russian prose the
  last comma is nearly always inside a sentence. On the fixture sweep the old code
  recovered 0 elements from a truncation that had 2 complete questions behind it.

Both are gone. `_scan_json` walks the response once, tracking string literals and
escapes, and returns the open-delimiter stack plus the offsets where a nested value
finished. `parse_json_response` now tries `json.loads` FIRST (twice — as-is, then
with trailing commas dropped) and only what genuinely fails reaches the repair, which
tries candidates longest-first: close the stack where the text stopped, else cut back
to each completed element, newest first. A salvage that would yield `[]`/`{}` from a
response that plainly carried content returns `None` instead — "nothing found" and
"nothing survived" must not look alike.

**The 2026-08-17 entry below says these shapes "all parse".** They did, as single
replayed strings. What that replay missed is that the tail-cut to the last `}` was
doing the work, and it stops helping the moment the truncation lands inside the FIRST
element. Fixture sweeps over every truncation offset, not hand-picked samples.

**Nobody was reading `finish_reason`.** Every provider reports hitting the output
ceiling and `ai_client.py` ignored all of them, so a cut-off answer travelled on as
complete and surfaced as a JSON complaint. `_warn_if_truncated` now covers the local,
OpenAI-compatible, Anthropic and Google paths; the phrase it prints reaches the
pipeline through the child's stderr, which prefixes the user-facing error with "ответ
обрезан лимитом токенов". Verified against a live llama.cpp with `--max-tokens 12`.

**One flaky feature no longer costs the run.** Each feature gets one second attempt
(`_feature_attempts`), gated to unparsable ANSWERS — a provider error or timeout has
already spent `ai_client`'s retry budget and re-running it just doubles a wait that is
already minutes long. `server/processing/worker.py` got the same, gated on the
`ValueError` it raises for the schema. The unparsable text is written next to the
artifacts as `<stem>_failed_<feature>.txt`: only the first 300 characters ever reached
the user, which is why this run could not be diagnosed from what it left behind.
`_selftest_analysis_retry.py` drives all of it end to end through the real pipeline
with a fake AI CLI made deliberately flaky (7 checks).

**Found by the suite, not by the run:** `obsidian_reads_a_dotted_date` had been
failing before any of this. `meeting_datetime_from_name` did `Path(name).stem` on
input that was already a stem, so `Планёрка 17.08.2026 15-33` lost `.2026 15-33` and
the note got today's date. It now strips only a tail that looks like a real extension.

Suite after all of it: 51 runners, 1581 checks, zero failures. Version deliberately
unchanged at 1.2.1 — this was a debug pass, not a release.

## The protocol invented its own date, and a real-data audit (2026-08-17)

**Measured across the owner's 14 real analyses in `transcripts/`:** the formal
protocol's metadata was hallucinated in every single one — "24.10.2023" as the date
in ten of them, "Текущая дата (на основании транскрипции)" in one, and a
differently-shaped number each time (`01-2023-PR`, `ПМ-2023-10-24-01`, `PROT-2023-
10-24-001`, `Протокол совещания № 2`). Meanwhile the truth was in the file name:
every recording is `2026-08-17 15-33-43.mkv`.

`media.meeting_datetime_from_name()` is now the ONE parser for it (the two private
copies in `obsidian.py` and `gsheets.py` delegate to it and stopped being limited to
`YYYY-MM-DD`): it reads year-first, day-first, compact and `T`-separated forms, takes
the time that follows the date, and validates the ranges so `v1.2.2026` or
`2026-13-45 99-99-99` parse as nothing. `analysis.protocol_facts()` turns that into
the three fields — date in the analysis output language (RU documents get
17.08.2026), `time` as start–end using `parse_duration_label` + `shift_clock` on the
recorded duration, and `protocolNumber` = date + start time, which invents nothing
and stays unique per meeting. The facts are stated to the model
(`protocol_facts_block`, inserted immediately before the transcript so
`protocolText` carries them too) AND written over the parsed fields
(`apply_protocol_facts`) — a model that ignores instructions must not be the reason
a protocol is dated three years wrong. When the name has NO date, nothing is
imposed and the model's own answer stands, exactly as before. Desktop and cabinet
share it (`server/processing/worker.py` passes the same facts). 47 checks in
`_selftest_protocol.py`, including end-to-end through the pipeline.

**Bottom controls used half the window.** All eleven of them huddled against the
transcript column with the right half of the section empty. Now two full-width rows
with the meeting's own actions on the left and the destination on the right
(Спикеры | Экспорт…, По спикерам + Перегенерировать | база знаний), and the status
line on its own wrapping row. Fitting mattered: the export group plus three buttons
demanded 1160 px against a 1116 px budget, so "По спикерам" moved down a row —
container minimum 1131 px, no horizontal scrollbar at the default size.

**A cancelled run leaked 328 MB.** `transcripts/2026-07-28 14-31-45 (2)/` held a
171 MB `_temp.wav` and nine `_temp_chunk_*.wav` and nothing else — the folder's
mtime matches the `job … failed: __cancelled__` line in `logs/app.log`. Two causes:
`processor.py` deleted only the full WAV on its error path (never the chunks), and a
CANCELLED run is killed outright so it never reaches any cleanup at all. Fixed at
both ends — the processor's error path globs `<stem>_temp*.wav`, and
`JobRunner._sweep_temp_audio()` runs on cancel and on failure, immediately and once
more 2 s later (Windows refuses to unlink a file the dying process still holds).
Strictly scoped to this job's own temp WAVs; a check asserts the artifacts survive.

**Verified, not assumed, in the same audit:** the two `actionItems` failures in the
log ("Вот JSON массив…" + fences, and a truncated array) are recovered by the
current `_strip_fences_and_tail`/`_fix_incomplete_json` — replayed against the real
shapes from the log, all five parse.

**A user cancel was logged as ERROR.** `job … failed: __cancelled__` made a
deliberate action look like a defect and inflated every error count. `main.py` now
routes the outcome through `log_job_finished()`: ok → INFO, `__cancelled__` → INFO
("cancelled by the user"), a real error → ERROR. Checked in `_selftest_ui`.

**The Google Sheets export is healthy — and a timed-out POST still wrote its row.**
Verified against the owner's live spreadsheet: 18/18 columns match `HEADERS`, every
one of the 12 `done` meetings has a row, the cancelled one correctly has none, and
the meeting whose export raised `TimeoutError` after 30 s (`2026-07-22 10-33-33`)
has exactly ONE fully-populated row. So the request WAS applied and only reading
the answer timed out — which settles the design question: a retry on timeout would
duplicate rows, because the webhook has no idempotency key. Do not add one.

*Method note, worth more than the finding:* the first pass at this audit concluded
the opposite — "ten rows missing, the script must be bound to another spreadsheet".
That came from reading the CSV export through a summarising fetch, which quietly
stopped after ~13 lines. The file is 3407 PHYSICAL lines and 25 logical rows: every
summary cell contains newlines, so a CSV of this sheet can only be counted with a
real CSV parser. Parse structured data; never eyeball it through a summariser.

What did need fixing: only failures were ever logged, so "my row is missing" could
not be told apart from "the export never ran" — the question that sent this audit
down the wrong path in the first place. Both optional exports now log their SUCCESS
too (the webhook's answer, the written Obsidian notes).

## The queue was not a history, and Regenerate was all-or-nothing (2026-08-17)

Three defects the owner hit in one session, on the last file of a queue whose
analysis died on a single feature.

**Regenerate could only redo BOTH.** The run had a good summary and a broken
analysis, and the only button available rebuilt the summary as well — minutes of
local-model time to get back to where the file already was. `Перегенерировать` is
now a menu of three scopes, and the pipeline honours each one:
`JobRunner.start_from_transcript(path, scope)` with `'summary'` (stop after the
summary), `'analysis'` (skip the summary, reuse the newest recorded version and
build only the analysis) or `'both'` (the old behaviour). `PipelineQueue`'s pending
tuple carries the scope, so a queued regeneration cannot lose it. Two outcomes that
used to be silent are now failures with a reason: an analysis-only run with every
section switched off (`an_no_features`) and a summary-sourced analysis with no
summary on disk (`an_no_summary`) — the second is also blocked in the UI before
anything is queued. An analysis-only run spans the whole progress bar instead of
starting at the 50% mark it inherited from the full pipeline.

**Divergent version counts, audited.** Scoped regeneration makes "summary v2 +
analysis v3" (and the reverse) normal, so every consumer of the two lists was
re-read: nothing pairs them by index — `history.add_*_version` counts each list on
its own, both pickers clamp with `max(0, min(idx, len-1))`, `_version_at` clamps,
export names carry the version of THEIR kind, and `source_summary_version` is
passed explicitly by `_finish_analysis` (an analysis-only run links to the newest
summary that actually exists). `mcp_server`/`rag` only ever take the latest. 14
checks cover `2 summaries / 3 analyses` and `3 summaries / 1 analysis` end to end.
One real defect surfaced there: a summary-only run holds **no** analysis in memory,
and both optional auto-exports published that emptiness — a Google Sheets row with
every analysis column blank for a meeting that has an analysis. `_analysis_for_export()`
now falls back to the newest recorded version, and the Obsidian auto-export passes
explicit `kinds`, so a scoped run never writes an empty note named after a version.

**A long failure reason put a HORIZONTAL scrollbar on the window.** Measured, not
guessed: `lbl_status` had no word wrap, so a 3-line provider error made it demand
**3783 px** and the body scrolled sideways by 2669 px. The label now wraps (with
`QSizePolicy.Ignored` horizontally, or wrapping alone still lets it push), the
status section's `setFixedHeight(220)` became `setMinimumHeight(220)` so the panel
grows DOWNWARDS to show the wrapped text, and the Details cell elides (word wrap
off, `ElideRight`) with the full text in its tooltip. Two more width leaks found on
the way: `lbl_export_status` (export/RAG errors, same treatment) and the six-button
actions row, which demanded 1258 px — more than the window's own default 1200 —
and is now split into meeting actions + knowledge base. Container minimum width:
**3857 px → 1131 px**, no horizontal scrollbar at the default size. The timeline
also escapes the error now: it is rendered as rich text and provider messages quote
the model's own `<think>` output, which Qt was eating as markup.

**The queue WAS the history, so cleaning the queue erased it.** `history.json` is
the archive of meetings (artifacts, versions) and the table is rebuilt from it, so
`store.remove()` took the only record of the processing with it — after a cleanup
there was nothing that said when a file was processed, how long it took, or why it
failed. The journal is now a separate file, `config/processing_history.json`
(`desktop/app/core/run_history.py`): one record per RUN — kind (full / summary /
analysis), start, finish, duration, every status transition with its timestamp,
every finished stage with its seconds, artifacts produced, provider/engine, and the
error. Written by `JobRunner` itself (hooked to its own `status_changed` /
`stage_done` signals, so nothing reported to the UI is missing from the log) and
flushed on every transition, so a crash still leaves the trail; runs left open by a
kill are stamped `interrupted` at the next start (`mark_interrupted`, called from
`build_app`). Read in a new window — 🕘 in the header,
`desktop/app/ui/history_dialog.py` — runs newest-first with a filter, and a detail
pane with the status history, stage timings and the whole wrapped error text. The
store is optional on `JobRunner`/`MainWindow`, so tests and embedders are
unaffected. 44 checks in `_selftest_run_history.py`, including that the journal
survives `store.remove()` and that the window needs no horizontal scroll after a
long error.
...
set QT_QPA_PLATFORM=offscreen && backend\python\python.exe desktop\_selftest_run_history.py  # processing journal + scoped regenerate + long-error wrapping
backend\python\python.exe desktop\_selftest_protocol.py   # protocol metadata from the file name (date/time/number), not invented
...
backend\python\python.exe desktop\_livetest_live.py       # live mode on real devices: plays speech into the loopback, checks it comes back as text
```
