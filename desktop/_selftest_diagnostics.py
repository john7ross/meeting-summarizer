...
# The desktop client showed its version nowhere, so a bug report could not name
# the build it came from. It must be the version the manifest declares, not a
# second copy of the number.
import json as _json                                        # noqa: E402
from desktop.app.version import APP_VERSION                 # noqa: E402
_manifest = _json.loads(
    (ROOT / "package.json").read_text(encoding="utf-8"))["version"]
check("diagnostics_shows_the_app_version",
      APP_VERSION in dlg._lbl_version.text(), dlg._lbl_version.text())
check("shown_version_is_the_manifest_version", APP_VERSION == _manifest,
      f"{APP_VERSION} vs {_manifest}")
