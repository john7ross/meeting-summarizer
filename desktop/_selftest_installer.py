...
# so must this, or every self-test run leaves another msbuild_* behind in %TEMP%.
...
for _dir in {fp.parent for fp, _ in _min_items + _full_items}:
    _shutil.rmtree(_dir, ignore_errors=True)
for _name in ("RUN.bat", "SERVER.bat"):
    check(f"min_{_name.lower()}_prefers_the_recorded_interpreter",
          "config\\interpreter.txt" in _min[_name] and '"%PY%"' in _min[_name],
          "must not hardcode bare `python`")
    check(f"min_{_name.lower()}_still_falls_back_to_python",
          'set "PY=python"' in _min[_name],
          "a user who skipped INSTALL.bat must still get a sensible attempt")
    check(f"full_{_name.lower()}_uses_the_bundled_runtime",
          "backend\\python\\python.exe" in _full[_name],
          "the full build carries its own interpreter and must not consult PATH")
