...
def _cancelling_is_not_logged_as_a_failure():
    """Pressing Cancel is a decision, not a defect.

    It used to reach the Diagnostics log as ``ERROR job … failed: __cancelled__``,
    so a deliberate action looked exactly like a crash — and every scan of the log
    counted it as one.
    """
    import logging
    from app.main import log_job_finished
    ...
    log = logging.getLogger("selftest.jobfinished")
    log.propagate = False
    log.setLevel(logging.INFO)
    log.addHandler(_Capture())
    try:
        log_job_finished(log, 1, True, "")
        log_job_finished(log, 2, False, "__cancelled__")
        log_job_finished(log, 3, False, "Local API error: status=503")
    finally:
        log.handlers.clear()

    levels = [level for level, _ in lines]
    assert levels == [logging.INFO, logging.INFO, logging.ERROR], levels
    assert "cancelled" in lines[1][1] and "__cancelled__" not in lines[1][1], lines[1][1]
    assert "503" in lines[2][1], lines[2][1]
    return lines[1][1]


check("cancelling_is_not_logged_as_a_failure", _cancelling_is_not_logged_as_a_failure)
