...
from .models import (SOURCE_FILE, AnalysisVersion, HistoryEntry, JobStatus,
                     SummaryVersion, normalize_source)
...
class HistoryStore:
    ...
    def add(self, video_path, duration: str = "", size: str = "",
            source: str = SOURCE_FILE) -> int:
        """Register a meeting. ``source`` is the intake CHANNEL ('file' / 'live')
        and is recorded so the queue, the journal and the profile can tell a live
        meeting from a processed file long after the fact."""
