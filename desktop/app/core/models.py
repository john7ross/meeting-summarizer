...
# Intake channels. A meeting arrives either as a recording that already exists
# (dropped file, added URL, microphone recording processed as a file) or live —
# recorded in the app and processed from the transcript live mode produced.
SOURCE_FILE = "file"
SOURCE_LIVE = "live"
SOURCES = (SOURCE_FILE, SOURCE_LIVE)

SOURCE_LABELS = {
    "ru": {SOURCE_FILE: "Файл", SOURCE_LIVE: "Live-встреча"},
    "en": {SOURCE_FILE: "File", SOURCE_LIVE: "Live meeting"},
}

# Short marker for dense places (the queue row, the journal table).
SOURCE_BADGE = {SOURCE_FILE: "", SOURCE_LIVE: "● live"}


def normalize_source(value) -> str:
    """Coerce anything to a known channel; unknown/missing => 'file'.

    Legacy entries have no channel at all, and every one of them came from a
    file — defaulting to 'file' keeps them truthful rather than 'unknown'.
    """
    text = str(value or "").strip().lower()
    return text if text in SOURCES else SOURCE_FILE


def source_label(value, language: str = "ru") -> str:
    table = SOURCE_LABELS.get(language, SOURCE_LABELS["ru"])
    return table[normalize_source(value)]


def source_badge(value) -> str:
    return SOURCE_BADGE[normalize_source(value)]
...
class HistoryEntry:
    ...
    # How this meeting entered the app — the intake CHANNEL, not the file type:
    #   "file" — a recording that already existed (video, audio, or a URL download)
    #   "live" — recorded here with live transcription, and processed from the
    #            text live mode produced instead of transcribing the file again
    # It changes nothing about WHAT is produced (deliberately: the artifact set is
    # identical either way) and everything about being able to tell the two apart
    # afterwards — in the queue, in the journal, in the logs and in the profile.
