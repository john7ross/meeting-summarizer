...
class DiagnosticsDialog(QDialog):
    ...
    def _build_system_tab(self) -> QWidget:
        w = QWidget()
        v = QVBoxLayout(w)
        # The application version had no home in the desktop UI at all: the web
        # cabinet shows it in the footer, exported reports carry it, and someone
        # reporting a problem from the desktop client had no way to say which
        # build they were on. Diagnostics is exactly where that question is asked.
        from ..version import APP_VERSION
        self._lbl_version = QLabel(f"Meeting Summarizer v{APP_VERSION}")
        self._lbl_version.setObjectName("sectionTitle")
        self._lbl_version.setTextInteractionFlags(
            Qt.TextInteractionFlag.TextSelectableByMouse)
        v.addWidget(self._lbl_version)
        self._bars = {}
        self._vals = {}
