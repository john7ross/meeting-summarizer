...
from PySide6.QtWidgets import (
    ...
    QLabel, QLineEdit, QMainWindow, QMenu, QMessageBox, QPlainTextEdit, QProgressBar, QProgressDialog,
...
from ..core.models import (SOURCE_LIVE, JobStatus, main_label, source_badge,
                           source_label)
...
LABELS = {
    "ru": {
        ...
        "tip_history": "История обработки — журнал всех прогонов: когда, сколько по "
                       "времени, этапы, статусы, ошибки. Не зависит от очереди.",
        ...
        "regen_tip": "Отредактируйте транскрипт слева (он редактируемый) и выберите, "
                     "что пересобрать: только саммари, только анализ или и то и другое. "
                     "Правки сохраняются, создаются НОВЫЕ версии — старые остаются.",
        "regen_summary": "Только саммари",
        "regen_analysis": "Только анализ",
        "regen_both": "Саммари + анализ",
        "regen_summary_tip": "Новая версия саммари из текущего транскрипта. "
                             "Анализ не трогаем.",
        "regen_analysis_tip": "Новая версия анализа — по транскрипту (или по "
                              "последнему саммари, если так задано в настройках). "
                              "Саммари не пересоздаётся: то, что нужно, когда прогон "
                              "упал на одном из разделов анализа.",
        "regen_both_tip": "Новое саммари, затем новый анализ по нему.",
        "regen_confirm_title": "Перегенерация",
        "regen_confirm": "Создать новую версию саммари и анализа из текущего транскрипта?",
        "regen_confirm_summary": "Создать новую версию САММАРИ из текущего транскрипта?",
        "regen_confirm_analysis": "Создать новую версию АНАЛИЗА? Саммари останется прежним.",
        "regen_running": "Перегенерация…",
        "regen_running_summary": "Пересоздаю саммари…",
        "regen_running_analysis": "Пересоздаю анализ…",
        ...
        "source_is": "Источник: {name}",
        "live_import_failed": "Не удалось взять live-транскрипт: {err}",
        "live_import_empty": "Live-транскрипт пуст — обрабатывать нечего. Запустите обычную обработку записи.",
        "regen_no_summary": "Нет саммари: для анализа по саммари сначала создайте саммари",
    ...
    "en": {
        ...
        "tip_history": "Processing history — the journal of every run: when, how "
                       "long, stages, statuses, errors. Independent of the queue.",
        ...
        "regen_tip": "Edit the transcript on the left (it is editable) and pick what to "
                     "rebuild: the summary only, the analysis only, or both. Your edits "
                     "are saved and NEW versions are created — the old ones stay.",
        "regen_summary": "Summary only",
        "regen_analysis": "Analysis only",
        "regen_both": "Summary + analysis",
        "regen_summary_tip": "A new summary version from the current transcript. "
                             "The analysis is left alone.",
        "regen_analysis_tip": "A new analysis version — from the transcript (or from "
                              "the latest summary, if the settings say so). The summary "
                              "is not rebuilt: what you want when a run died on one "
                              "analysis section.",
        "regen_both_tip": "A new summary, then a new analysis from it.",
        "regen_confirm_title": "Regenerate",
        "regen_confirm": "Create a new summary and analysis version from the current transcript?",
        "regen_confirm_summary": "Create a new SUMMARY version from the current transcript?",
        "regen_confirm_analysis": "Create a new ANALYSIS version? The summary stays as it is.",
        "regen_running": "Regenerating…",
        "regen_running_summary": "Rebuilding the summary…",
        "regen_running_analysis": "Rebuilding the analysis…",
        ...
        "live_import_failed": "Could not take the live transcript: {err}",
        "live_import_empty": "The live transcript is empty - there is nothing to process. Run normal processing on the recording instead.",
        "regen_no_summary": "No summary: create one first for a summary-sourced analysis",
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, settings: dict, store: HistoryStore, queue=None,
                 ...
                 persist_ui_preferences: bool = False, run_store=None):
        super().__init__()
        self.settings = settings
        self.store = store
        self.queue = queue
        # Journal of runs (config/processing_history.json) — the history the queue
        # is NOT: it survives removing a meeting from the table. None => the 🕘
        # window opens on a default store, so the app still works when embedded
        # without one.
    ...
    def _build_header(self) -> None:
        ...
        self.btn_history = _icon_button("🕘", self._t("tip_history"))
    ...
    def _build_body(self) -> None:
        ...
        # A 600-character failure reason lands in the Details cell: elide it to the
        # column instead of letting the row demand the width (the full text is in
        # the tooltip, the status panel and the processing journal).
        ...
        # A failure reason is as long as the provider made it. Without wrapping the
        # label demanded ~3800 px of width, so a failed analysis put a HORIZONTAL
        # scrollbar on the whole window; it must wrap onto more lines instead and
        # let the section grow downwards (see the minimum height below).
        ...
        # The timeline remains complete and scrollable, so the section no longer
        # grows by one row for every completed stage — but it is a MINIMUM, not a
        # fixed height: a wrapped multi-line error has to be readable, and the
        # section grows in height to show it (never in width).
        s_frame.setMinimumHeight(220)
    ...
    def _section_results(self) -> tuple[QFrame, QVBoxLayout]:
        ...
        # Where this meeting came from. The artifacts are identical whichever
        # channel it arrived through, which is exactly why the channel has to be
        # stated somewhere the user is already looking at the result.
        self.lbl_source = QLabel("")
        self.lbl_source.setObjectName("hint")
        proj_row.addWidget(self.lbl_source)
        ...
        # Regenerate is a MENU, not one action: a run that died on a single
        # analysis feature must be able to redo the analysis alone, without
        # spending the model's time on a summary that is already good.
        self.btn_regenerate = QPushButton(self._t("regenerate"))
        self.btn_regenerate.setObjectName("regenButton")
        self.btn_regenerate.setEnabled(False)
        self.btn_regenerate.setToolTip(self._t("regen_tip"))
        self.menu_regenerate = QMenu(self.btn_regenerate)
        self.act_regen = {}
        for scope, key in (("summary", "regen_summary"),
                           ("analysis", "regen_analysis"),
                           ("both", "regen_both")):
            action = self.menu_regenerate.addAction(self._t(key))
            action.setToolTip(self._t(f"{key}_tip"))
            action.triggered.connect(
                lambda _checked=False, s=scope: self._do_regenerate(s))
            self.act_regen[scope] = action
        self.menu_regenerate.setToolTipsVisible(True)
        self.btn_regenerate.setMenu(self.menu_regenerate)
        ...
        # Export/RAG failures put a provider message in here: it must wrap like the
        # status line, never widen the window.
        self.lbl_export_status.setWordWrap(True)
        self.lbl_export_status.setSizePolicy(QSizePolicy.Policy.Ignored,
                                             QSizePolicy.Policy.Preferred)
        # Two rows that USE the full width instead of huddling against the
        # transcript column: what acts on the meeting's content sits on the left,
        # where that content goes sits on the right, and the stretch between them
        # takes the slack. Keeping the eleven controls on one row demanded ~1260 px
        # — more than the window's own default width — so it scrolled sideways.
        exp.addWidget(self.btn_speakers)
        exp.addStretch(1)
        ...
        # "По спикерам" rides with Regenerate rather than with Speakers: both
        # PRODUCE something from this meeting, and the export group next to it is
        # wide enough that a third button on that row overflowed the window by 46 px.
        for w in (self.btn_export_speakers, self.btn_regenerate):
            actions_row.addWidget(w)
        actions_row.addStretch(1)
        for w in (self.btn_add_rag, self.btn_rag, self.btn_search):
            actions_row.addWidget(w)
        outer.addLayout(actions_row)
        # The status line gets its own full-width row: it wraps, and squeezing it
        # between buttons is what let a long export error stretch the window.
        outer.addWidget(self.lbl_export_status)
    ...
    def add_job_row(self, entry, *, restored: bool = False) -> None:
        ...
        path_text = entry.video_path or entry.video_name
        # The intake channel travels with the row: a live meeting and a dropped
        # file produce the same artifacts on purpose, which is exactly why the
        # queue has to say which of the two this was.
        badge = source_badge(getattr(entry, "source", ""))
        file_item = QTableWidgetItem(f"{badge}  {path_text}" if badge else path_text)
        file_item.setToolTip(
            f"{source_label(getattr(entry, 'source', ''), self.language)}\n{path_text}")
    ...
    def _stages_from_history(self, entry) -> list:
        ...
        reason = (getattr(entry, "error", "") or "").strip()
        if reason:
            lines.append("✖ " + escape(reason).replace("\n", "<br>"))
        if not lines:
    ...
    def on_finished(self, job_id, ok, error) -> None:
        ...
        was_regenerating = (
            ...
            and self.lbl_export_status.text() in {
                self._t("regen_running"), self._t("regen_running_summary"),
                self._t("regen_running_analysis")})
        ...
        if error == "__cancelled__":
            ...
            details = self.table.item(row, self.COL_DETAILS)
            details.setText(msg.replace("\n", "  "))   # one elided line + tooltip
            details.setToolTip(msg)
            # The timeline is rendered as rich text, and a provider error quotes the
            # model's own output — '<think>' and friends would be eaten as markup.
            self._stages_by_job.setdefault(job_id, []).append(
                "✖ " + escape(msg).replace("\n", "<br>"))
            state = self._live_by_job.setdefault(job_id, {})
            state.update({
            ...
            if job_id == self._active_job:
                ...
                # Wrapped, not truncated to one line — but a whole stderr dump
                # belongs in the timeline and the journal, not in the status line.
                shown = msg if len(msg) <= 600 else msg[:600] + "…"
                self.lbl_status.setText(
                    f"{name}: {main_label(JobStatus.ERROR, self.language)} — {shown}")
    ...
    def _load_results(self, job_id) -> None:
        ...
        self.edit_project.blockSignals(False)
        self._retranslate_source_label(entry)
        self._current_transcript = ""
    ...
    def _retranslate_source_label(self, entry=None) -> None:
        """Render 'Source: …' for the loaded meeting, in the current language."""
        if entry is None:
        ...
        if entry is None:
            self.lbl_source.setText("")
            return
        self.lbl_source.setText(self._t("source_is").format(
            name=source_label(getattr(entry, "source", ""), self.language)))
    ...
    # -- regenerate ---------------------------------------------------
    def _do_regenerate(self, scope: str = "both") -> None:
        """Rebuild *scope* ('summary' / 'analysis' / 'both') for the loaded meeting.

        Whatever the scope, the transcript shown on screen is saved first: it is
        editable, and the regenerated artifacts must derive from exactly what the
        user is looking at.
        """
        from PySide6.QtWidgets import QMessageBox
        if self._current_result_id is None:
            return
        scope = scope if scope in ("summary", "analysis", "both") else "both"
        ...
        if not entry:
            return
        # An analysis built FROM the summary cannot run before there is one; say so
        # instead of quietly analysing the transcript and calling it the same thing.
        if (scope == "analysis" and not entry.summary_versions
                and self.settings.get("analysisSource", "transcript") != "transcript"):
            self.lbl_export_status.setText(self._t("regen_no_summary"))
            return
        confirm = QMessageBox.question(
            self, self._t("regen_confirm_title"),
            self._t({"summary": "regen_confirm_summary",
                     "analysis": "regen_confirm_analysis"}.get(scope, "regen_confirm")),
        ...
        if self.queue:
            self.btn_regenerate.setEnabled(False)
            self.lbl_export_status.setText(
                self._t({"summary": "regen_running_summary",
                         "analysis": "regen_running_analysis"}.get(scope, "regen_running")))
            self.queue.enqueue_regenerate(
                entry.id, entry.video_path, transcript_path, scope)
    ...
    def _open_recorder(self) -> None:
        ...
        Settings are handed in (and written back) because the recorder now owns
        real settings — system audio, live transcription, live summary — and the
        dialog must not keep a private copy that drifts from settings.json.
        """
        from .recorder_dialog import RecorderDialog
        dlg = RecorderDialog(paths.ROOT, language=self.language, parent=self,
                             settings=self.settings,
                             on_settings_changed=self._on_recorder_settings)
        if not (dlg.exec() and dlg.result_path):
            return
        if dlg.use_live_transcript and dlg.live_transcript_path:
            self._add_live_recording(dlg.result_path, dlg.live_transcript_path)
        else:
            self._add_files([dlg.result_path])

    def _add_live_recording(self, recording_path: str, transcript_path: str) -> None:
        """Queue a recording whose transcript live mode already produced.

        The ONLY thing that differs from the normal flow is where the transcript
        came from. It is written into the job folder under the same
        ``<stem>_raw.txt`` name a transcription run would have written, recorded
        on the entry the same way, and then the same summary → analysis passes
        run over it — so the meeting ends up with exactly the artifact set every
        other meeting has (transcript, summary version, analysis version, the
        exports, Obsidian, Sheets). A different route to the input must not
        produce a different-looking result.

        Trimming is skipped: the live transcript covers the whole recording, so
        cutting the audio into several meetings would leave each of them with a
        transcript of all the others as well.
        """
        try:
            text = Path(transcript_path).read_text(encoding="utf-8", errors="replace")
        except OSError as exc:
            QMessageBox.warning(self, self._t("title"),
                                self._t("live_import_failed").format(err=exc))
            return
        if not text.strip():
            QMessageBox.warning(self, self._t("title"),
                                self._t("live_import_empty"))
            return

        entry_id = self.store.add(recording_path,
                                  duration=self._probe_duration_label(recording_path),
                                  size=self._file_size_label(recording_path),
                                  source=SOURCE_LIVE)
        entry = self.store.get(entry_id)
        if not entry:
            return
        raw_path = self.store.job_dir(entry_id) / f"{Path(entry.video_name).stem}_raw.txt"
        try:
            ...
            return
        try:
            self.store.set_transcript(entry_id, raw_path)
        except KeyError:
            pass
        self.add_job_row(entry)
        if self.queue:
            self.queue.enqueue_regenerate(entry_id, recording_path, str(raw_path), "both")
        else:
            self._pending.append((entry_id, recording_path))

    def _on_recorder_settings(self, settings: dict) -> None:
        """Persist a toggle flipped in the recorder window."""
        self.settings.update(settings or {})
        try:
            config.save_settings(self.settings)
        except OSError:
            pass          # a failed save must not interrupt a live recording

    def _open_history(self) -> None:
        """The processing journal — every run, independent of what the queue holds."""
        from .history_dialog import HistoryDialog
        store = self.run_store
        if store is None:
            from ..core.run_history import RunHistoryStore
            store = RunHistoryStore()
        HistoryDialog(store, language=self.language, parent=self).exec()
    ...
    def retranslate(self) -> None:
        ...
        for job_id in [j for j in self._stages_by_job
            ...
            self._stages_by_job.pop(job_id, None)
        self.setWindowTitle(self._t("title"))
        # The intake channel is rendered from data, so it has to be re-rendered
        # rather than re-labelled — it stays in the old language otherwise.
        self._retranslate_source_label()
        ...
        self.btn_history.setToolTip(self._t("tip_history"))
        ...
        self.txt_raw.setToolTip(self._t("regen_tip"))
        for scope, key in (("summary", "regen_summary"),
                           ("analysis", "regen_analysis"), ("both", "regen_both")):
            action = self.act_regen.get(scope)
