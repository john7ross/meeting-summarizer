...
LABELS = {
    "ru": {
        ...
        "g_live": "Запись и live-режим",
        "recSystem": "Писать системный звук вторым каналом",
        "recSystem_tip": "Захват того, что звучит из динамиков (WASAPI loopback): на онлайн-"
                         "встрече в запись попадают и собеседники, а не только вы. "
                         "Микрофон пишется в левый канал, системный звук — в правый; "
                         "итоговая транскрибация сводит их в моно как обычно.",
        "liveTr": "Live-транскрибация во время записи",
        "liveTr_tip": "Речь распознаётся прямо во время встречи выбранным движком. "
                      "Это черновик: файл записи всё равно проходит обычную обработку "
                      "после остановки, и итоговый транскрипт делается заново.",
        "liveSum": "Live-саммари во время записи",
        "liveSum_tip": "Сводка встречи, которая дополняется по ходу разговора. "
                       "Требует включённой live-транскрибации.",
        "liveEngine": "Движок для live:",
        "liveEngine_same": "Как основной",
        "liveModel": "Модель для live (пусто = как основная):",
        "liveEngine_tip": "Можно оставить основным движком medium на GPU для итогового "
                          "транскрипта, а live гонять малой моделью на процессоре.",
        "liveInterval": "Интервал обновления сводки (сек):",
        "liveStrategy": "Стратегия обновления сводки:",
        "liveStrategy_auto": "Авто (по провайдеру)",
        "liveStrategy_regen": "Всегда пересобирать из транскрипта",
        "liveStrategy_hybrid": "Гибрид: инкремент + пересборка",
        "liveStrategy_tip": "Инкремент дешевле, но ошибка сводки закрепляется в следующих "
                            "обновлениях. Пересборка из транскрипта этого не допускает, но "
                            "тратит больше токенов. «Авто» — пересборка на локальной модели "
                            "(токены бесплатны) и гибрид на облаке.",
        "liveMaxUpdates": "Лимит обновлений сводки (0 = без лимита):",
        "liveMaxUpdates_tip": "Защита от неожиданного счёта на платном провайдере: после "
                              "N обновлений сводка перестаёт обновляться, запись продолжается.",
        "liveNotSupported": "Основной движок не умеет live; выберите движок для live отдельно.",
        "recSystemOff": "Системный звук недоступен: {reason}",
        "recSystemNoPackage": "не установлен пакет soundcard",
        "recSystemNoDevice": "нет устройства вывода с поддержкой захвата",
        "recSystemOn": "Будет записываться вывод устройства: {name}",
    ...
    "en": {
        ...
        "chunkChars_tip": "Threshold in CHARACTERS of the transcript text. If the transcript is "
                          ...
                          "0 = use the default.",
        "g_live": "Recording & live mode",
        "recSystem": "Record system audio as the second channel",
        "recSystem_tip": "Captures what your speakers play (WASAPI loopback): on an online "
                         "meeting the recording holds the other participants too, not just "
                         "you. The microphone goes to the left channel and system audio to "
                         "the right; the final transcription downmixes them as usual.",
        "liveTr": "Live transcription while recording",
        "liveTr_tip": "Speech is recognised during the meeting by the selected engine. "
                      "It is a draft: the recorded file still goes through normal "
                      "processing after you stop, and the final transcript is made afresh.",
        "liveSum": "Live summary while recording",
        "liveSum_tip": "A meeting summary that grows as the conversation goes on. "
                       "Requires live transcription.",
        "liveEngine": "Engine for live:",
        "liveEngine_same": "Same as the main engine",
        "liveModel": "Model for live (empty = same as main):",
        "liveEngine_tip": "You can keep medium on the GPU as the main engine for the final "
                          "transcript and run live on a small CPU model.",
        "liveInterval": "Summary update interval (sec):",
        "liveStrategy": "Summary update strategy:",
        "liveStrategy_auto": "Auto (by provider)",
        "liveStrategy_regen": "Always rebuild from the transcript",
        "liveStrategy_hybrid": "Hybrid: incremental + rebuilds",
        "liveStrategy_tip": "Incremental is cheaper, but a mistake in the summary is carried "
                            "into every later update. Rebuilding from the transcript prevents "
                            "that and costs more tokens. 'Auto' rebuilds on a local model "
                            "(tokens are free) and goes hybrid on a cloud one.",
        "liveMaxUpdates": "Summary update limit (0 = unlimited):",
        "liveMaxUpdates_tip": "A guard against a surprise bill on a metered provider: after "
                              "N updates the summary stops updating; recording continues.",
        "liveNotSupported": "The main engine cannot run live; pick a live engine separately.",
        "recSystemOff": "System audio unavailable: {reason}",
        "recSystemNoPackage": "the soundcard package is not installed",
        "recSystemNoDevice": "no output device supports loopback capture",
        "recSystemOn": "Will record the output of: {name}",
...
class SettingsDialog(QDialog):
    ...
    def _build_live(self) -> None:
        """Recording sources and the live (during-the-meeting) draft.

        Grouped together because they answer one question — what happens WHILE
        the meeting runs — as opposed to everything else here, which is about
        what happens to the file afterwards.
        """
        g = self._group("g_live")
        self.chk_rec_system = QCheckBox(self._t("recSystem"))
        self.chk_rec_system.setToolTip(self._t("recSystem_tip"))
        g.addWidget(self.chk_rec_system)
        self.lbl_system_state = QLabel("")
        self.lbl_system_state.setObjectName("hint")
        self.lbl_system_state.setWordWrap(True)
        g.addWidget(self.lbl_system_state)

        self.chk_live_tr = QCheckBox(self._t("liveTr"))
        self.chk_live_tr.setToolTip(self._t("liveTr_tip"))
        g.addWidget(self.chk_live_tr)
        self.chk_live_sum = QCheckBox(self._t("liveSum"))
        self.chk_live_sum.setToolTip(self._t("liveSum_tip"))
        g.addWidget(self.chk_live_sum)

        self.cb_live_engine = _combo(self._live_engine_options())
        self.cb_live_engine.setToolTip(self._t("liveEngine_tip"))
        self.w_live_engine = _labeled_row(self._t("liveEngine"), self.cb_live_engine)
        g.addWidget(self.w_live_engine)
        self.ed_live_model = QLineEdit()
        self.w_live_model = _labeled_row(self._t("liveModel"), self.ed_live_model)
        g.addWidget(self.w_live_model)
        self.lbl_live_support = QLabel(self._t("liveNotSupported"))
        self.lbl_live_support.setObjectName("warning")
        self.lbl_live_support.setWordWrap(True)
        g.addWidget(self.lbl_live_support)

        self.sp_live_interval = self._spin(10, 3600)
        self.w_live_interval = _labeled_row(self._t("liveInterval"), self.sp_live_interval)
        g.addWidget(self.w_live_interval)
        self.cb_live_strategy = _combo([
            ("auto", self._t("liveStrategy_auto")),
            ("regen", self._t("liveStrategy_regen")),
            ("hybrid", self._t("liveStrategy_hybrid"))])
        self.cb_live_strategy.setToolTip(self._t("liveStrategy_tip"))
        self.w_live_strategy = _labeled_row(self._t("liveStrategy"), self.cb_live_strategy)
        self.w_live_strategy.setToolTip(self._t("liveStrategy_tip"))
        g.addWidget(self.w_live_strategy)
        self.sp_live_max = self._spin(0, 100000)
        self.sp_live_max.setToolTip(self._t("liveMaxUpdates_tip"))
        self.w_live_max = _labeled_row(self._t("liveMaxUpdates"), self.sp_live_max)
        g.addWidget(self.w_live_max)

    def _live_engine_options(self) -> list:
        """'same as the main engine' plus every engine with a streaming adapter."""
        from ..backend import live as live_backend
        options = [("", self._t("liveEngine_same"))]
        labels = {e["id"]: self._engine_label(e) for e in self._catalog_engines}
        for engine in live_backend.live_engines():
            options.append((engine, labels.get(engine, engine)))
        return options
    ...
    def _load(self) -> None:
        ...
        self.chk_rec_system.setChecked(bool(s.get("recordSystemAudio", False)))
        self.chk_live_tr.setChecked(bool(s.get("liveTranscription", False)))
        self.chk_live_sum.setChecked(bool(s.get("liveSummary", False)))
        _combo_set(self.cb_live_engine, str(s.get("liveEngine", "") or ""))
        self.ed_live_model.setText(str(s.get("liveModel", "")))
        self.sp_live_interval.setValue(_as_int(s.get("liveSummaryInterval"), 30))
        _combo_set(self.cb_live_strategy, s.get("liveSummaryStrategy", "auto"))
        self.sp_live_max.setValue(_as_int(s.get("liveSummaryMaxUpdates"), 0))
    ...
    def _wire(self) -> None:
        ...
        self.chk_rec_system.toggled.connect(lambda *_: self._refresh_system_audio_state())
        self.chk_live_tr.toggled.connect(self._on_live_transcription_toggled)
        self.chk_live_sum.toggled.connect(self._apply_visibility)
        self.cb_live_engine.currentIndexChanged.connect(self._apply_visibility)
        # The live engine may be "same as main", so a change of the MAIN engine
        # can turn live on or off. Recheck rather than leave a stale warning.
        self.cb_engine.currentIndexChanged.connect(self._apply_visibility)

    def _on_live_transcription_toggled(self, on: bool) -> None:
        if not on:
            self.chk_live_sum.setChecked(False)
        self._apply_visibility()

    def _refresh_system_audio_state(self) -> None:
        """Say whether loopback capture can actually run on THIS machine.

        A checkbox that can be ticked but does nothing is the worst of both
        worlds; the probe is cheap and its answer is specific.
        """
        from ..core.loopback import default_output_name, probe as probe_loopback
        available, reason = probe_loopback()
        self.chk_rec_system.setEnabled(available)
        if not available:
            self.chk_rec_system.setChecked(False)
            self.lbl_system_state.setText(self._t("recSystemOff").format(
                reason={"no-package": self._t("recSystemNoPackage"),
                        "no-device": self._t("recSystemNoDevice")}.get(reason, reason)))
            return
        self.lbl_system_state.setText(
            self._t("recSystemOn").format(name=default_output_name() or "-")
            if self.chk_rec_system.isChecked() else "")

    def _live_engine_in_use(self) -> str:
        return _combo_val(self.cb_live_engine) or _combo_val(self.cb_engine) or ""
    ...
    def _apply_visibility(self, *_) -> None:
        ...
        # -- live -------------------------------------------------------
        from ..backend import live as live_backend
        live_on = self.chk_live_tr.isChecked()
        self.chk_live_sum.setEnabled(live_on)
        for w in (self.w_live_engine, self.w_live_model):
            w.setVisible(live_on)
        summary_on = live_on and self.chk_live_sum.isChecked()
        for w in (self.w_live_interval, self.w_live_strategy, self.w_live_max):
            w.setVisible(summary_on)
        self.lbl_live_support.setVisible(
            live_on and not live_backend.supports_live(self._live_engine_in_use()))
    ...
    def _collect(self) -> dict:
        ...
        updated.update({
            ...
            "recordSystemAudio": self.chk_rec_system.isChecked(),
            "liveTranscription": self.chk_live_tr.isChecked(),
            # Live summary without a live transcript has no input. Saving it as
            # "on" would leave a switch that claims to be enabled and does
            # nothing, which is worse than saying no.
            "liveSummary": (self.chk_live_sum.isChecked()
                            and self.chk_live_tr.isChecked()),
            "liveEngine": _combo_val(self.cb_live_engine) or "",
            "liveModel": self.ed_live_model.text().strip(),
            "liveSummaryInterval": self.sp_live_interval.value(),
            "liveSummaryStrategy": _combo_val(self.cb_live_strategy),
            "liveSummaryMaxUpdates": self.sp_live_max.value(),
