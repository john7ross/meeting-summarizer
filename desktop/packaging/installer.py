...
T = {
    "ru": {
        ...
        "ffmpeg_hint": "Без ffmpeg не получится извлечь звук из записи. Скачайте сборку для Windows\n"
                       "с https://ffmpeg.org/download.html и положите ffmpeg.exe и ffprobe.exe\n"
                       "в backend\\FFmpeg, либо добавьте их в PATH.",
        "ffmpeg_step": "ffmpeg (нет в этой копии - скачается)",
    ...
    "en": {
        ...
        "ffmpeg_hint": "Without ffmpeg the audio cannot be extracted from a recording. Download a\n"
                       "Windows build from https://ffmpeg.org/download.html and put ffmpeg.exe and\n"
                       "ffprobe.exe into backend\\FFmpeg, or add them to PATH.",
        "ffmpeg_step": "ffmpeg (missing from this copy - will be downloaded)",
...
CORE_PACKAGES = ["PySide6", "numpy", "requests", "huggingface-hub", "yt-dlp",
                 ...
                 # System-audio capture for the recorder. Pure Python on top of
                 # cffi/numpy, which are already here, so it costs nothing to
                 # install and its absence would silently halve every recorded
                 # online meeting.
                 "soundcard"]
...
SIZE_MB = {"core": 350, "torch_cuda": 2800, "torch_cpu": 350, "rag": 900,
           "server": 60, "pyannote": 250, "ffmpeg": 385}
...
def build_plan(selection: dict, pins: dict, server_pins: dict) -> dict:
    ...
    # A git clone has no backend/FFmpeg - 385 MB of binaries GitHub refuses (its
    # hard limit is 100 MB per file and both of ours are 193 MB). Without them the
    # install succeeds and the first audio extraction then fails, so fetch them.
    # The archives already carry ffmpeg, so this only fires for a source checkout.
    need_ffmpeg = not ffmpeg_present()
    if need_ffmpeg:
        size += SIZE_MB["ffmpeg"]
    return {"steps": steps, "models": models, "local_llm_model": llm,
            "size_mb": size, "torch_index": torch_index, "ffmpeg": need_ffmpeg}
...
def print_plan(plan: dict, p: dict, t: dict) -> None:
    print(f"\n{t['plan']}:")
    if plan.get("ffmpeg"):
        print(f"  -> {t['ffmpeg_step']} ({_fmt_mb(SIZE_MB['ffmpeg'])})")
...
def run_plan(plan: dict, t: dict) -> int:
    total = (len(plan["steps"]) + len(plan["models"])
             + (1 if plan["local_llm_model"] else 0)
             + (1 if plan.get("ffmpeg") else 0))
    n = 0
    if plan.get("ffmpeg"):
        n += 1
        print(f"\n[{t['step']} {n}/{total}] {t['installing']}: ffmpeg")
        if not install_ffmpeg():
            print(f"\n{t['failed']} {n}/{total}: ffmpeg")
            print(t["ffmpeg_hint"])
            return 1
...
# BtbN's builds are the ones ffmpeg.org links to for Windows, and the `-gpl`
# variant carries the same licence terms as the binary the archives ship, which
# is what THIRD-PARTY-NOTICES describes.
FFMPEG_URL = ("https://github.com/BtbN/FFmpeg-Builds/releases/download/latest/"
              "ffmpeg-master-latest-win64-gpl.zip")
FFMPEG_DIR = ROOT / "backend" / "FFmpeg"


def ffmpeg_present() -> bool:
    return (FFMPEG_DIR / "ffmpeg.exe").is_file() and (FFMPEG_DIR / "ffprobe.exe").is_file()


def install_ffmpeg() -> bool:
    """Fetch ffmpeg/ffprobe into backend/FFmpeg. Returns True on success."""
    import io                                    # noqa: PLC0415
    import urllib.request                        # noqa: PLC0415
    import zipfile                               # noqa: PLC0415

    # Streamed in chunks with a progress line rather than one blocking read().
    # The archive is ~170 MB; on a throttled link that read() sits silent for
    # over an hour and the console looks hung, while every other step of the
    # install reports progress. Measured at 32 KB/s during a release check.
    try:
        buf = io.BytesIO()
        with urllib.request.urlopen(FFMPEG_URL, timeout=120) as resp:
            ...
            while True:
                ...
                print(f"\r  {_fmt_mb(got // 1024 // 1024)}{pct}   ",
                      end="", flush=True)
        print()
        blob = buf.getvalue()
        ...
        with zipfile.ZipFile(io.BytesIO(blob)) as zf:
            for entry in zf.namelist():
                ...
                if name in wanted:
                    with zf.open(entry) as src, open(FFMPEG_DIR / name, "wb") as dst:
                        shutil.copyfileobj(src, dst)
                    wanted.discard(name)
        if wanted:
            print(f"  [!] not found inside the archive: {', '.join(sorted(wanted))}")
            return False
    except Exception as exc:                     # noqa: BLE001
        print(f"  [!] {exc}")
        return False

    # Record what was actually installed, so the licence paperwork describes the
    # binary on disk rather than the one we happened to ship.
    try:
        conf = subprocess.run([str(FFMPEG_DIR / "ffmpeg.exe"), "-hide_banner", "-buildconf"],
                              capture_output=True, text=True, encoding="utf-8",
                              errors="replace", timeout=120)
        (FFMPEG_DIR / "BUILD-CONFIGURATION.txt").write_text(
            f"Downloaded by INSTALL.bat from:\n{FFMPEG_URL}\n\n"
            f"{conf.stdout or conf.stderr}", encoding="utf-8")
    except Exception:                            # noqa: BLE001 - a missing note is not fatal
        pass
